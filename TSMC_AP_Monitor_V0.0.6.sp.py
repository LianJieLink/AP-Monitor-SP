...
def generate_3d_html(data, origin, optimal_trace):
    ...
    # layout change: only 1 row (Map)
    fig = make_subplots(rows=1, cols=1, specs=[[{"type": "map"}]])
    ...
    for m in range(NUM_MEMBERS):
        ...
        # Map Trace (Top) - NOW ONLY THIS
        fig.add_trace(go.Scattermap(
            lon=dummy_v, lat=dummy_v, mode="markers+lines", name=f"Trace {m+1}",
            line=dict(width=2, color=color), opacity=0.4,
            visible='legendonly',
            hoverinfo="text",
            showlegend=False
        ), row=1, col=1)
    ...
    # REMOVED: Uncertainty Band logic
    # REMOVED: Static Optimal Trace logic (Will be processed dynamically)
    ...
    fig.update_layout(
        map=dict(style="open-street-map", center=dict(lat=origin_lat, lon=origin_lon), zoom=10),
        shapes=[dict(type="line", xref="paper", yref="paper", x0=0.02, y0=0.03, x1=0.12, y1=0.03, line=dict(color="#000000", width=2))],
        annotations=[dict(xref="paper", yref="paper", x=0.02, y=0.045, text="Scale", showarrow=False, font=dict(color="#000000", size=12, family="Consolas"), xanchor="left")]
    )
    
    # REMOVED: Axis updates for Row 2
