...
*   **3D 軌跡分析 (Trajectory Analysis)**：
    ...
    *   **最佳路徑切換**：可一鍵顯示/隱藏最佳路徑及其不確定性區間 (Uncertainty Bands)。
    *   **視覺優化**：軌跡與高度剖面圖皆以 `markers + lines` 模式顯示，並提供詳細懸停資訊 (經度、緯度、高度、時間)。
...
*   **V0.0.6**:
    ...
    *   **功能增強**：
        ...
        *   **視覺增強**：軌跡圖與高度剖面圖全面採用 `markers + lines` 顯示，並新增詳細 Hover 資訊 (含時間、經緯高)。
        *   **最佳路徑 (Optimal Trace) 優化**：
            *   改為**半透明紅色虛線**樣式 (Simulated Dotted Line)，以區分一般成員軌跡。
            *   實作 **路徑加密 (Path Densification)** 演算法 (5x Upsampling)，解決點狀虛線過於稀疏的問題，提供更平滑的視覺效果。
